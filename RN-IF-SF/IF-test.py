...
def RunModel(_abnormal_data, _abnormal_label, _hidden_num, _elem_num, _file_name, _partition):
    ...
    for j in range(ensemble_space):
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            if save_model:
                save_path = saver.save(sess, './saved_model/' + pathlib.Path(_file_name).parts[0] + '/masked_skip_rnn_seq2seq_' + str(cell_type) + '_' + os.path.basename(_file_name) + '.ckpt')
    ...
    if not partition:
        ...
        np.savetxt('./saved_result/' + pathlib.Path(_file_name).parts[0] + '/masked_skip_rnn_seq2seq_' + os.path.basename(_file_name) + '_score.txt', score_pred_label, delimiter=',')  # X is an array
